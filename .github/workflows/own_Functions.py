...
def mousePoints(event,xmouse,ymouse,flags,params):
    """
    print mouse coordinates (computer mouse not mouse mouse) when click on frame 
    (e.g. used to calibrate the patch locations).

    cv2.setMouseCallback("frame", mousePoints)

    output: x, y position of mouse
    """
...
def solenoID(currentPatch, currentTrapeze):
    ...
    valve#, int
    """
    valvePatch = {'NW':0, 'NE':4, 'SW':8, 'SE':12} # with NE = object 3, NW = 4, SE = 1 and SW= 2, can be used to only reward one object, have to also change line 241 to add condition 'and currentPatch[1] == g_o'
